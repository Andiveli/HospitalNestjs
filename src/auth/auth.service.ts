...
export class AuthService {
    ...
    async compararPassword(password: string, confirmPassword: string): Promise<boolean> {
    ...
    async guardar(usuario: AuthInterface, enviarEmail: boolean = false) {
        ...
        if(existe) {
            if(enviarEmail) {
                ...
                await this.mailService.recuperarEmail(existe.email, existe.nombre, usuario.token);
            }
        ...
        await this.mailService.enviarEmail(nuevoUsuario.email, nuevoUsuario.nombre, nuevoUsuario.token);
        return resultado;
    }
    ...
    async confirmarUsuario(token: string) {
        ...
        throw new HttpException('Token inválido o usuario no encontrado', HttpStatus.BAD_REQUEST);
    }
    ...
    async usuarioConfirmado(email: string) {
        ...
        if(usuario) {
           if(usuario.confirmado) {
            ...
            throw new HttpException('El usuario no ha sido confirmado', HttpStatus.UNAUTHORIZED);
        }
