...
export class AuthController {
    ...
    async confirmarUsuario(@Param('token') token: string ) : Promise<{msg: string}> {
    ...
    @HttpCode(HttpStatus.OK)
    async olvidePassword(@Body('email') email : string) : Promise<{msg: string}> {
    ...
    @HttpCode(HttpStatus.OK)
    async comprobarToken(@Param('token') token: string) : Promise<{msg: string}> {
    ...
    @HttpCode(HttpStatus.OK)
    async restablecer(@Param('token') token: string, @Body('password') password: string, @Body('confirmPassword') confirmPassword: string) : Promise<{msg: string}> {
